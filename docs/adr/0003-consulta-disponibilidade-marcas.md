...
Falta a ponta oposta do problema: a consulta **proativa** de disponibilidade, ou seja, verificar se um nome novo está livre para uso e depósito *antes* de lançar um produto, uma marca ou uma linha de negócio. Hoje essa checagem é manual e informal.

O cenário de acesso ao dado do INPI é limitado:

- O INPI não expõe uma API pública documentada de busca de marcas.
- A nova plataforma de busca (`servicos.busca.inpi.gov.br`) tem uma API interna, mas não documentada, não versionada e protegida por hCaptcha. O uso programático depende de engenharia reversa contra um alvo que pode mudar sem aviso.
...
- A tabela `registro_rpi_publicacoes` cresce continuamente e exige atenção a volume e indexação ao longo do tempo.
