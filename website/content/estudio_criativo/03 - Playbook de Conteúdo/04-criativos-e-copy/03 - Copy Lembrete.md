---
title: Copy - Lembrete
summary: "Define copy de lembrete como urgência organizada (não insistência emocional): recolocar o compromisso, tornar o prazo concreto e reduzir procrastinação com prazo, consequência e ação imediata. Vale para datas reais em campanha e para follow-up em operação contínua."
topics: [copy, lembrete, urgência real, prazo, decisão, follow-up]
keywords: [lembrete, urgência organizada, prazo, consequência, ação imediata, decisão, fricção, prioridade, ciclo de campanha, follow-up, recuperação de leads, escassez real]
priority: medium
ai_when_to_use: |
  Use quando o usuário for escrever copy de lembrete para reativar leads que já conhecem o produto/evento, precisar comunicar prazo final ou últimas vagas, ou quiser transformar intenção acumulada em execução.
related: []
---

# Copy - Lembrete

[Mapa de Criativos \- Lembrete](https://docs.google.com/spreadsheets/d/1dBd8ji3-cQdaHL0yGirmvraedFstDFJSwbJ4dgkD2fU/edit?gid=235710671#gid=235710671)

## Criativos de lembrete existem para reativar decisões. Aqui, o público já conhece o evento, o produto ou a proposta. O papel da peça não é explicar tudo de novo. É reduzir fricção e trazer a prioridade de volta para o centro.

Se captação trabalha percepção e escolha, **lembrete trabalha urgência organizada.** Nesse ciclo precisamos de objetividade absoluta, tempo visível e dizer exatamente o que fazer ou saber agora.

**A função principal do criativo de lembrete é simples:**

* Recolocar o compromisso na frente da pessoa.  
* Tornar o prazo concreto.  
* Diminuir a procrastinação.

A copy, nesse caso, precisa ser ainda mais direta. É momento de foco.

**Estrutura típica:**

* Prazo claro e verdadeiro.  
* Consequência real (o encontro acontece naquele dia; a turma começa; o acompanhamento tem capacidade).  
* Ação imediata.

**Exemplo estrutural:**

* “O encontro é quinta, às 20h.”  
* “A turma começa dia 12; depois disso, só na próxima.”  
* “Sua inscrição está aberta neste link.”

**A escassez que usamos é a que existe de verdade.** Uma turma tem tamanho. Uma mentoria tem capacidade humana. Um encontro ao vivo tem data. Esses limites são reais e podem ser ditos com todas as letras. **Fora deles, não inventamos limite nenhum:** nada de “vagas limitadas” quando não são, “última chance”, contador regressivo falso, FOMO ou promessa de faturamento. Urgência fabricada é a forma mais rápida de destruir a confiança que a captação levou meses para construir.

O erro mais comum em criativos de lembrete é voltar para o modo explicativo. Isso dilui urgência. Se a pessoa ainda precisa entender tudo de novo, o problema está no ciclo anterior. Visualmente, a peça deve reduzir o ruído. O design serve para destacar prazo e decisão, não para introduzir novos argumentos.

Lembrete não é insistência emocional. É uma organização de prioridade. Ele existe para transformar intenção acumulada em execução concreta.

## Quando esta peça se aplica

Lembrete se aplica sempre que existe uma **data real** à frente: um encontro ao vivo, o início de uma turma, o fim de uma inscrição que realmente fecha, uma sessão com hora marcada. <dado q="A" />

A formulação original desta página assumia que lembrete era uma etapa fixa do ciclo de lançamento, entre captação e carrinho aberto. Isso descrevia a operação anterior da empresa. <dado q="D" />

Com a migração para aquisição e vendas contínuas, lembrete deixa de ser uma etapa do calendário e passa a ser um **tipo de peça acionado por evento**. Lançamentos continuam acontecendo, e nesses casos a página vale integralmente como está. <dado q="B" />

**O que muda em operação contínua:**

* **Produtos perpétuos não têm prazo e, por isso, não pedem lembrete de prazo.** Para eles, a peça equivalente é o **follow-up**: retomar a conversa onde ela parou, responder a objeção que ficou pendente, mostrar o que mudou desde o último contato. A função continua sendo reduzir fricção, não criar pressão.
* **Recuperação de leads substitui a contagem regressiva.** Quem demonstrou interesse e não avançou merece uma retomada honesta (“você olhou isso em março; desde então mudou X”), não um alarme.
* **A consequência precisa ser verificável.** Em ciclo, a consequência era o fechamento. Em operação contínua, é a capacidade real: turma com tamanho definido, agenda de mentoria com limite humano, encontro com data fixa. Se não existe consequência real, não existe lembrete: existe outro tipo de peça.
* **Frequência é uma decisão de relacionamento.** Sem o fim do carrinho como freio natural, o excesso de lembretes passa a ser o risco principal. Mais silêncio do que ruído.
