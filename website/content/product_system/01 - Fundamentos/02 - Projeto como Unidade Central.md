---
title: Projeto como Unidade Central
summary: Descreve a migração de conteúdo para projeto como unidade central da experiência Overlens. Explica a lógica Projeto → necessidade → conhecimento → aplicação → evidência, o que muda na relação entre aulas, livros, artigos e trilhas, e o que o produto precisa conseguir fazer para sustentar essa inversão.
topics: [projeto, unidade central, filosofia educacional, produto, aprendizagem, evidência]
keywords: [projeto como unidade central, conteúdo, trilha, catálogo, necessidade, conhecimento, aplicação, evidência, inversão, Overpass, aprendizagem aplicada, biblioteca de cursos]
priority: high
ai_when_to_use: |
  Use quando o usuário perguntar por que a Overlens trata projeto como unidade central em vez de conteúdo, o que isso significa na prática para aulas e trilhas, ou como funciona a lógica Projeto → necessidade → conhecimento → aplicação → evidência.
related: []
---

# Projeto como Unidade Central

## A pergunta que organiza o produto mudou de "o que você quer aprender?" para "o que você quer construir, realizar ou se tornar?". Tudo o que vem depois é consequência disso.

A direção está definida na tese da companhia; o que ainda não foi validado é a entrega. <dado q="B" fonte=".claude/rules/tese-atual.md" />

A Overlens está migrando de **conteúdo como unidade central** para **projeto como unidade central**. É uma frase curta com consequência larga, e ela é frequentemente mal lida das duas formas opostas: como se significasse abandonar conteúdo, ou como se fosse apenas uma nova forma de falar do mesmo catálogo.

Não é nenhuma das duas. **Aulas, livros, artigos e trilhas continuam existindo.** O que muda é a relação entre eles, e quem determina a ordem.

## A inversão

A lógica desejada:

> **Projeto → necessidade → conhecimento → aplicação → evidência**

No modelo anterior, a pessoa consumia conhecimento organizado por tema e depois procurava onde usá-lo. O conteúdo vinha primeiro e o destino, se aparecesse, aparecia depois. A consequência mais comum não é ignorância, é acúmulo: muita informação disponível, pouca capacidade instalada.

No modelo que a Overlens persegue, a pessoa parte do que deseja realizar. O projeto declara uma necessidade, a necessidade convoca o conhecimento, o conhecimento é aplicado, e a aplicação deixa evidência.

A diferença prática está em quem define a relevância. No catálogo, a relevância é definida pela curadoria. No projeto, é definida pela distância entre onde a pessoa está e o que ela quer que exista.

## O que a Overlens precisa conseguir responder

Se o projeto é a unidade, o produto precisa ajudar a descobrir:

| Pergunta | O que ela destrava |
| :---- | :---- |
| **O que preciso aprender?** | Converte um objetivo difuso em repertório identificável. |
| **Quais capacidades preciso desenvolver?** | Separa o que é conhecimento do que é habilidade demonstrável. |
| **Quais ferramentas utilizar?** | Reduz o custo de entrada da execução. |
| **Quais pessoas podem ajudar?** | Torna a comunidade parte do caminho, e não um espaço paralelo. |
| **Como executar?** | Transforma intenção em sequência de passos com entregáveis. |
| **Como validar?** | Impede que esforço seja confundido com resultado. |
| **Como melhorar?** | Fecha o ciclo e devolve a pessoa ao início com mais capacidade. |

Nenhuma dessas sete perguntas é respondida hoje de forma sistemática pelo produto. Elas descrevem a ambição, não o estado. <dado q="C" />

## O que isso não significa

**Não significa fim do conteúdo.** Conteúdo continua sendo insumo, e bom conteúdo continua caro de produzir. O que muda é que ele deixa de ser o índice da experiência e passa a ser convocado por uma necessidade.

**Não significa que todo mundo chega com um projeto.** Muita gente chega com vontade e sem objeto. Parte do trabalho do produto é ajudar a nomear o projeto, o que é diferente de exigir que ele exista na entrada.

**Não significa que fundamentos desaparecem.** Há repertório que precisa vir antes de qualquer execução. Forçar projeto onde ele não cabe produz projeto decorativo, que é pior do que aula honesta.

**Não significa gamificação.** Projeto não é missão, e evidência não é troféu. O valor do registro está no que ele prova fora da plataforma.

## Por que essa inversão sustenta o produto

Dois motivos, ambos verificáveis em princípio:

**Conhecimento aplicado gera resultado observável.** Um projeto em andamento produz artefatos, decisões e consequências. Um módulo concluído produz um registro de que alguém chegou ao fim de um vídeo. Só o primeiro permite dizer o que a pessoa consegue fazer.

**Um projeto em andamento é um motivo recorrente para voltar.** Catálogo é consumido e abandonado; projeto tem próximo passo. Isso importa para o produto antes de importar para o negócio: uma superfície que precisa ser interessante todo dia é diferente de uma superfície que precisa estar disponível quando alguém lembrar dela.

## O que precisa ser verdade

Para que a inversão funcione: <dado q="C" />

1. Que seja possível transformar um objetivo vago em projeto delimitado sem atendimento individual.
2. Que o percurso continue útil para quem ainda não sabe o que quer construir.
3. Que as pessoas registrem o que fizeram. Sem registro, não há evidência, e sem evidência a última etapa do ciclo não existe.
4. Que a estrutura suporte projetos de naturezas muito diferentes, já que o público é definido pelo estado e não pela profissão.

## O que está em aberto

- O que qualifica algo como projeto dentro da Overlens: escopo mínimo, duração, critério de conclusão. <dado q="E" />
- Como um projeto se relaciona com as ofertas existentes na prática, e não apenas no conceito. <dado q="E" />
- O que acontece com projetos abandonados: se somem, se viram histórico ou se continuam contando como evidência parcial. <dado q="E" />

A estrutura que liga aprendizagem, projeto e execução está descrita em `Fundamentos › PBL`.
