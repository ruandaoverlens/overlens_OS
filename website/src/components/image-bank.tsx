      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
        {IMAGES.map((asset) => (
          <ImageCard
            ...
            onToggleFavorite={() => toggleFavorite({ id: asset.filename, type: "image", title: asset.title, subtitle: `${asset.author}, ${asset.year}`, thumbnail: imageSrc(asset.filename) })}
          />
        ))}
      </div>

      {selected && (
        <ImageLightbox asset={selected} onClose={() => setSelected(null)} />
      )}
    </div>
  );
}
