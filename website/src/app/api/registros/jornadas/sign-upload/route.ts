...
export async function POST(request: NextRequest) {
  ...
  const { jornadaId, filename } = parsed.data;

  const { data: jornada } = await guard.supabase
  ...
  const storagePath = `jornadas/${jornadaId}/${Date.now()}-${sanitizeStorageFilename(filename)}`;
  ...
  if (error || !data) {
    console.error("[registros/jornadas/sign-upload]:", error?.message);
